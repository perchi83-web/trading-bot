    ...
    for intento in range(1, reintentos + 1):
        try:
            exchange = ccxt.binance({
                ...
                "options": {"defaultType": config.MODO_MERCADO},
            })
...
# ============================================================
def calcular_riesgo(precio_entrada, atr, direccion="LONG"):
    """
    Gestión de riesgo dinámica basada en ATR — Ratio 1:3
    LONG:  Stop-Loss ABAJO del precio, Take-Profit ARRIBA
    SHORT: Stop-Loss ARRIBA del precio, Take-Profit ABAJO
    """
    capital_op     = config.CAPITAL_TOTAL * config.RIESGO_POR_OPERACION
    distancia_stop = atr * 1.8

    if direccion == "SHORT":
        stop_loss   = precio_entrada + distancia_stop       # SL arriba del precio
        take_profit = precio_entrada - (distancia_stop * 3) # TP abajo del precio
    else:
...
def evaluar_senal(df_1h, df_4h, simbolo):
    ...
    if tendencia_1h == "ALCISTA":
        ...
        # ---- SEÑAL SHORT: tendencia 1h BAJISTA + RSI sobrecomprado + BB superior ----
        if rsi > config.RSI_SOBRECOMPRADO and bb_cerca_superior and macd_bajista:
            tipo      = "SHORT"
            confianza = "ALTA"
            razon     = f"1h BAJISTA + RSI {rsi} (sobrecompra) + cruce MACD bajista + precio en BB superior"

        elif rsi > config.RSI_SOBRECOMPRADO and bb_cerca_superior:
            tipo      = "SHORT"
            confianza = "MEDIA"
            razon     = f"1h BAJISTA + RSI {rsi} (sobrecompra) + precio en BB superior"

        elif rsi < 20 and bb_cerca_inferior:
            tipo      = "COMPRA"
            confianza = "BAJA"
    ...
    # Si 4h es ALCISTA no abrir SHORT (contra-tendencia mayor)
    # -------------------------------------------------------
    ...
    if tipo == "SHORT" and tendencia_4h == "ALCISTA":
        tipo      = "ESPERAR"
        confianza = "-"
        razon     = f"FILTRO MTF: 4h ALCISTA anula señal SHORT en 1h"

    # Calcular riesgo con ATR dinámico — SHORT invierte SL/TP
    direccion = "SHORT" if tipo == "SHORT" else "LONG"
    riesgo = calcular_riesgo(precio, atr, direccion) if tipo in ["COMPRA", "VENTA", "SHORT"] else None

    # Construir mensaje Telegram
    if tipo in ["COMPRA", "VENTA", "SHORT"] and riesgo:
...
def ejecutar_ciclo():
    ...
    for simbolo in config.SIMBOLOS:
        try:
            # Establecer apalancamiento al inicio de cada análisis (solo futuros)
            try:
                exchange.set_leverage(config.LEVERAGE, simbolo)
                log.info(f"{simbolo}: apalancamiento establecido a {config.LEVERAGE}x")
            except Exception as e_lev:
                log.warning(f"{simbolo}: no se pudo establecer leverage — {e_lev}")
            ...
            if senal["tipo"] in ["COMPRA", "VENTA", "SHORT"]:
                ...
                if en_cooldown:
                    ...
                    # Gemini razona la señal antes de alertar
                    veredicto_ia = agente_ia.obtener_analisis_ia(senal)
                    log.info(f"Veredicto Gemini para {simbolo}: {veredicto_ia}")

                    if "sí" in veredicto_ia.lower():
                        paper_trading.abrir_operacion(
                            senal["simbolo"],
                            senal["precio"],
                            senal["tipo"],
                            {"decision": senal["tipo"], "razon": veredicto_ia}
                        )
                        mensaje_final = (
                            f"{senal['mensaje']}\n"
                            f"{'='*32}\n"
                            f"🤖 VEREDICTO DE GEMINI:\n{veredicto_ia}"
                        )
                        enviar_alerta_con_botones(mensaje_final, senal)
                        _ultima_alerta[simbolo] = datetime.now()
                    else:
                        log.info(f"Gemini descartó la señal: {veredicto_ia}")
    ...
    for simbolo in config.STOCKS:
        try:
            ...
            if senal["tipo"] in ["COMPRA", "VENTA", "SHORT"] and senal["mensaje"]:
...
if __name__ == "__main__":
    log.info("Bot de trading v4 iniciado — Trend Following + RSI + SHORT (Futuros)")
    ...
    log.info(f"Mercado:    {config.MODO_MERCADO.upper()}")
